# A custom test for this project

################################################################################
# A mini-framework for testing
################################################################################
    ...
    for q in questions:
        ...
        for testq, points, fn in TESTS:
            ...
            try:
                ...
                tracker.unmute()
                print("\n\nCaught KeyboardInterrupt: aborting test")
                tracker.finalize()
                print("\n[test was interrupted before finishing]")
                sys.exit(1)
            except:
                tracker.unmute()
...
def verify_node(node, expected_type, expected_shape, method_name):
    if expected_type == 'parameter':
        ...
        assert False, "If you see this message, please report a bug in the test"
...
def check_perceptron(tracker):
    ...
    for multiplier in (-5, -2, 2, 5):
        ...
        if np.abs(orig_weights).sum() == 0.0:
            # This test test doesn't work when weights are exactly zero
            continue
...
def check_regression(tracker):
    ...
    assert np.isclose(train_loss, sanity_loss), (
        ...
        "but the test computed a loss of {:.4f} "
